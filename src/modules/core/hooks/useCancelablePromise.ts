...
function makeCancelable(promise: any, name: any) {
  ...
  const wrappedPromise = new Promise<never>((resolve, reject) => {
...
export default function useCancelablePromise(debug = false) {
  const promises =
    useRef<{ promise: Promise<never>; name?: string; cancel: () => void }[]>();
   ...
   */
  function cancelablePromise(p: Promise<never>, name: string = "") {
